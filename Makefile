...
all: test_strobe test_encoder test_debounce test_pwm test_rgb_mixer 
...
test_rgb_mixer:
	...
	MODULE=test.test_rgb_mixer vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
...
test_strobe:
	rm -rf sim_build/; mkdir sim_build/
	iverilog -o sim_build/sim.vvp -s strobe_gen -s dump -g2012 src/strobe_gen.v test/dump_strobe.v
	MODULE=test.test_strobe vvp -M $$(cocotb-config --prefix)/cocotb/libs -m libcocotbvpi_icarus sim_build/sim.vvp
	! grep failure results.xml
